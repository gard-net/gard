...
static void PrintUsage()
{
    ...
    garc — reference CLI for the Landspeed Protocol (LSP/1.1).
    Measures throughput, latency, jitter and packet loss on a local network.

    usage:
      garc scan   [--seconds N]
      garc host   [--port P] [--name NAME]
      garc test   <host> [--port P] [--dir up|down|bidir]
                         [--duration S] [--streams N] [--payload BYTES]

    examples:
      garc scan --seconds 5
      garc host --port 7737
      garc test 192.168.1.50 --dir down --duration 10 --streams 4
    """);
}
